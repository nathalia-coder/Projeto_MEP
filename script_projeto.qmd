...
```{r modelo_sarima_09}
mod9 <- arima(serie_train,  c(2,1,1), seasonal = c(0,1,1))
coeftest(mod9) # tudo não significativo
mod9$aic 
```

Garante menor AIC e ruído brando nos resíduos

```{r}
acf(mod9$residuals)
pacf(mod9$residuals)
shapiro.test(mod9$residuals)
Box.test(mod9$residuals, lag = 12)
```

Também resulta em menor EQMP

```{r}
previsao_mod9 <- forecast(mod9, 12, level = 95)
EQMP_m9 <- sum((serie_test - previsao_mod9$mean)^2)/12
EQMP_m9
```
